...
app.get('/uploads/:folder/:filename', async (c) => {
  const { folder, filename } = c.req.param()
  const file = Bun.file(`./uploads/${folder}/${filename}`)
  if (await file.exists()) {
    return new Response(file, { headers: { 'Content-Type': file.type || 'application/octet-stream', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, max-age=3600' } })
  }
